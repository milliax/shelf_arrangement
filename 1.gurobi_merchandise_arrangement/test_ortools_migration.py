...
def test_ortools_solver():
    """Test the OR-Tools solver with sample data"""
    print("Testing OR-Tools solver migration...")
    ...
    # Create solver with None as db_manager for testing
    try:
            ...
            for shelf_id, items in solution.items():
                ...
                if items:
                    for item in items:
                        ...
                        print(f"  - {item['name']} {promoted} (${item['price']}, {item['width']}x{item['height']}x{item['depth']})")
                else:
                    print("  - Empty")
